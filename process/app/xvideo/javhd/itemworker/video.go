...
import (
	...
	"spider-go/process/app/xvideo/javhd/persist"
...
func doVideoWork(items *[]config.Item)  {
	for _, item := range *items {
		//var video model.Videos
		//
		item := item.(javhdConfig.Video)
		//
		//video.Name = item.Name
		//video.Status = model.VideoValidStatus

		dirPrefix, _ := uuid.NewV1()
		dir := fmt.Sprintf("%s/%s", javhdConfig.BaseFileDir, dirPrefix)
		...
		item.Mp4 = newMp4
		item.M3u8 = filesystem.Dir(newMp4) + "/" + javhdConfig.M3u8File
...
func SliceWithSaveVideoPipeLine() chan javhdConfig.Video {
	sliceVideoChan := make(chan javhdConfig.Video)
	go func() {
		for {
			item := <- sliceVideoChan
			...
			video.Name = item.Name
			video.Status = model.VideoValidStatus
			video.Mainimg = strings.TrimLeft(item.MainImg, "w:/")
			video.M3u8 = strings.TrimLeft(item.M3u8, "w:/")
			video.Mp4 = strings.TrimLeft(item.Mp4, "w:/")
			video.OriginalMp4 = video.Mp4

			long := 0
			if strings.Contains(item.Long, "h") {
				parseHours := strings.Split(item.Long, "h")
				hours, _ := strconv.Atoi(strings.Trim(parseHours[0], " "))
				long = hours * 3600
				item.Long = parseHours[1]
			} else if strings.Contains(item.Long, "min") {
				parseMins := strings.Split(item.Long, "min")
				mins, _ := strconv.Atoi(strings.Trim(parseMins[0], " "))
				long = long + mins * 60
				item.Long = parseMins[1]
			} else if strings.Contains(item.Long, "sec") {
				parseSecs := strings.Split(item.Long, "sec")
				secs, _ := strconv.Atoi(strings.Trim(parseSecs[0], " "))
				long = long + secs
			}
